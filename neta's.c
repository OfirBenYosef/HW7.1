...
grades* grades_init(){ 
	...
	grades_new-> students=(list*)(malloc(sizeof(list*)));
	if (!grades_new || !(grades_new-> students)) {
		/* Out of memory - must free what was allocated */
		free(grades_new);
		list_destroy(grades_new-> students);
		return NULL;
	}
...
grades* grades_init(){ 
    grades *grades_new;
    list **studets	grades_new=(grades*)(malloc(sizeof(grades)));
...
void student_data_destroy (void *elem){
	...
	free((to_be_destroy->student_grades));
	free(elem);
...
void grades_destroy(struct grades *grades){
	node *curr_node;
	student_data *curr_student;
	curr_node=list_begin(grades->students);

	while(curr_node){
		curr_student=list_get(curr_node);
		list_destroy(curr_student->student_grades);
...
/*
void grades_destroy(struct grades *grades){
	node *curr_student_node=list_begin(grades->students);
	student_data *curr_student_data=list_get(curr_student_node);
	while(curr_student_node){
		node *curr_course=list_begin(curr_student_data->student_grades);
		course_data *curr_course_data=list_get(curr_course);
		while(curr_course){
			course_data_destroy(curr_course_data);
			curr_course=list_next(curr_course);
		}
		student_data_destroy(curr_student_data);
		curr_student_node=list_next(curr_student_node);
		curr_student_data=list_get(curr_student_node);
	}
